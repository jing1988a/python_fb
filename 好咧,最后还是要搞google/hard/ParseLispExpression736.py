...
class Solution:
    def evaluate(self, expression):
        ...
        :rtype: int
        """
        # can i assime variable is int?
        variables=[{}]
        formatedExpression=self.reformat(expression)
        keywords=set('let' , 'mult' , 'add')
        return self.parse(formatedExpression , variables , keywords)
    def parse(self , expression , variables , keywords):
        if not expression:
            start=0
            end=len(expression)
            if expression[0]=='(':
            ...
            if type(expression[start])==int:
                variables.pop()
                return int(expression[start])
            if expression[start] not in keywords:
                variables.pop()
                return variables[-1][expression[start]]
            if expression[start]=='add':
                left , right=self.mySplit( expression[start+1:])
                variables.pop()
                return left+right
            if expression[start]=='mult':
                left, right = self.mySplit(expression[start + 1:])
                variables.pop()
                return left + right
            else:
